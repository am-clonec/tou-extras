...
public static class EvilSwapperEvents
{
    // lower number higher priority
    [RegisterEvent(10)]
    public static void ProcessVotesEventHandler(ProcessVotesEvent @event)
    {
        foreach (var swapper in CustomRoleUtils.GetActiveRolesOfType<EvilSwapperRole>())
        {
            SwapVotes(@event, swapper);
        }
    }
    ...
    {
        if (!CustomRoleUtils.GetActiveRolesOfType<EvilSwapperRole>().Any())
        {
            return;
        }
    ...
    private static void SwapVotes(ProcessVotesEvent @event, EvilSwapperRole swapper)
    {
    ...
    {
        var swapperRoles = CustomRoleUtils.GetActiveRolesOfType<EvilSwapperRole>().ToList();
        ...
        foreach (var role in swapperRoles)
        {
            ...
            if (swapPlayer1 == null || swapPlayer1.HasDied() || swapPlayer2 == null || swapPlayer2.HasDied())
            {
                yield break;
            }
    ...
    private static IEnumerator Slide2D(Transform target, Vector3 source, Vector3 dest, float duration)
    {
        ...
        while (elapsed < duration)
        {
            ...
            target.position = Vector3.Lerp(source, dest, t);
            yield return null;
        }
